...
# Seeking a more direct proxy for human activity for inm Central Park Warming
...
We then recreated a new linear model for TMAX (for 1900-2021 due to range of population data), attempting to use New York State `population` instead of year.   
...
We do observe a significant correlation (the p-value of the model's `population` coefficient is well below the cutoff of alpha = 0.05), the fit appears not to be any better than the regression against year. This caused us to wonder if a fit using month alone would be better than a fit using `month` or `year`, or `population` or `year`.  
...
The last question our team set out to address is whether a relationship exists between local weather and local human social and economic activity. The relationship has been been explored in the past with evidence suggesting weather has effects on an individuals mood, thermal comfort level, social interaction and can influence traffic, travel, public health, crime rates, and even stock prices (Horanont, 2013). Our team looked to expand on this prior work by making these observations at the local level of New York City. We decided to look specifically at the relationship between local weather and crime rates,stock prices, and public health. 

Two data sets were used to explore weather's impact on crime rates. First we looked at NYC shootings and then all arrests with 5,844 observations between 2006 and 2021. Both of these data sets were available through NYC's Open Data repository (https://opendata.cityofnewyork.us/).
...
We found records of all reported shootings, with at least one injured victim, occurring in New York City from 2006-2021. From here we extracted the number of daily shootings, and merged this information with our weather data from the same time frame. We conducted some simple EDA. We observed
...
We then completed a simple correlation plot, and observed negative correlations between number of shootings with `year` and `PRCP`, a positive correlation with `SNOW` and `TMAX`.  
...
These dependent variable-transformed models are of similar quality, both with an adjusted $R^2$ of 0.137, an improvement over the 0.128 value for the linear model with a non-transformed regressand (all models have F-statistic p-values of less than 0.05). Interestingly, in the variable-transformed models, the model coefficient for `SNOW` was not statistically significant.

## Arrest data

After looking at the relationship between reported shootings in NYC and local weather, we looked at arrests in New York (for any crime). This data was not previously analyzed by our group so we did some basic EDA to determine the distribution of the new data and identify correlations that exist.
...
We incorporated month and year into these relationships as we saw seasonality effects and changes over time in our previous effort. We plotted arrest counts and temperature, with points coded by precipitation as a binary factor (whether or not it rained or snowed) and month. This plot is below with no initial patterns that can be discerned. 
...
Observed correlations between crime and weather was the strongest from this analysis. This represents a valuable area for future study in the context of changing weather patterns that was explored in our early project.
