...
```{r formatting, echo = F}
DailyAQ_00_22$Date <- as.Date(DailyAQ_00_22$Date, format="%M/%d/%y")
...
ggplot(DailyAQ_00_22) +
  geom_histogram(aes(x=AQI), na.rm=TRUE, alpha=0.5, color="black", fill='#2DD164', bins=50, binwidth=3) +
  geom_vline(xintercept=mean_aqi, color="black", size=1, linetype=5, show.legend=FALSE) +
  annotate("text", x=mean_aqi + 9, y=450, label=paste(round(mean_aqi, 2)), angle=0, size=4, color="black") +
