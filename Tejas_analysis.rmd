...
ggplot(DailyAQ_00_22_Yearly_Growth, aes(group=1)) +
  ...
  geom_line(aes(x = year, y = aqi_diffRate), na.rm = T, stat="identity", color="#457108", size=1) +
  labs(title="Yearly average amount of PM2.5 particulates in atmosphere and growth rate", x="Year", y="ug/m3 LC") +
  scale_y_continuous(sec.axis = sec_axis(~., name="Year-over-year Diff (%)")) +
  theme(
...
From the pearson correlation plot above, we can see a significantly large, positive correlation between PM2.5 concentrations and the daily air quality index. Unfortunately, The correlations among our weather and air quality variables is relatively weak. We will still attempt a linear model between the variables below.   
...
Let's start by creating a linear model to describe the relationship between AQI and our weather variables.
...
summary(aqi_fit2)
xkabledply(aqi_fit2, title = paste("Model :", format( formula(aqi_fit2) )))
```
