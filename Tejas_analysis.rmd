...
# TODO: same figure, separate plots
ggplot(DailyAQ_00_22_Yearly_Growth, aes(group=1)) +
  ...
  labs(title="Average PM2.5 particulate amount and AQI by year in NYC", x="Year", y="ug/m3 LC") +
...
ggplot(DailyAQ_00_22_Yearly_Growth, aes(group=1)) +
  ...
  geom_point(aes(x = year, y = aqi_diffRate), na.rm = TRUE, fill="#E6E930", shape = 23) +
  labs(title="PM2.5 particulate and AQI growth rate by year in NYC", x="Year", y="ug/m3 LC") +
  theme(
...
Next, we will combine our new dataset with the NYC weather data based on the date. The days without a matching air quality measurement will be dropped after merge.
...
```{r corrplot, fig.align = "center", fig.}
...
From the pearson correlation plot above, we can see a significantly large, positive correlation between PM2.5 concentrations and the daily AQI values. This is expected as PM2.5 is significantly weighed in calculations of AQI. Unfortunately, the correlation significance among our weather and air quality variables is relatively weak. However, we will still attempt a linear model between them below.   

#### Linear models

Let's start by creating a linear model to describe the relationship between AQI and the variable we analyzed in our last project - TMAX - combined with the year.

```{r linear model}
aqi_fit <- lm(AQI ~ TMAX + TMIN + PRCP + SNOW, data = DailyAQ_numerics)
summary(aqi_fit)
xkabledply(aqi_fit, title = paste("First Linear Model: ", format( formula(aqi_fit) )))
```
The coefficients for each regressor is significant. TMIN and PRCP have a negative effect on daily AQI, while TMAX and SNOW have a positive effect on daily AQI. Although the p-value of the F-statistic is significant, the model regressors only explain a measly 0.9% of the variability in daily AQI measurements.  

Check for multicollinearity

```{r model 1 VIF}
# model VIF scores
xkablevif(aqi_fit, title = "VIF Of Model Covariates")
```

With a variance inflation factor of 12.4, we cannot accept TMAX and TMIN as regressors in our model.  

Reversed fit
```{r reversed model}
api_fit_rev <- lm(TMAX ~ PM2.5, data = DailyAQ_numerics)
summary(api_fit_rev)
xkabledply(api_fit_rev, title = paste("Model :", format( formula(api_fit_rev) )))
```
...
Let's build a model adding `month` as a categorical regressor and see how it affects the fit. Since we deemed the multicollinearity between TMAX and TMIN to be too high, TMIN is dropped from the model equation. We also go ahead and drop the PRCP and 
...
The regression coefficients for month and TMAX are all significant, with TMAX having a positive effect on daily AQI. Each month has a negative effect on daily AQI when compared to January, with September having the largest (negative) slope. The VIF values of all regressors are acceptable.  

The p-value of the model F-statistic is also significant, allowing us to reject the null hypothesis and conclude that there’s a significant relationship between our regressors, TMAX and month, and the daily AQI.  

However, the $R^2$ for our model is only `.149`. This indicates that only 14.7% of the variation in daily AQI can be explained by TMAX and month.  
...
Seasonality of TMAX and PM2.5/AQI?
```{r seasonality plots}
# TODO: combine into 1 figure
# TODO: remove TMIN
...
NYweath_Monthly_Avg %>%
  ...
  labs(title="Average Daily Max Temperature By Month", x="Month", y="Temperature (°F)") +
  scale_fill_manual(values=c("red", "blue"))

_Monthly_Avg <-  %>%
  group_by(month) %>%
  summarize("Avg AQI" = mean(TMAX, na.rm=T),
NYweath_Monthly_Avg %>%
  dplyr::select(month, "Avg Max Temp.", "Avg Min Temp.") %>%
  gather(key="Value", value="Temp", "Avg Max Temp.", "Avg Min Temp.") %>%
  ggplot(aes(x=month, y=Temp, fill=Value)) +
...
#ggplot(DailyAQ_merged) +
 #     geom_point(aes(x=DATE, y=PM2.5, color=month))
```
  
```{r scatter plots}
daq_length = length(DailyAQ_merged)

for (xx in 4:(daq_length - 2)) {
  for (yy in (xx+1):(daq_length - 1)) {
    p <- ggplot(DailyAQ_merged, aes(x=DailyAQ_merged[,xx], y=DailyAQ_merged[,yy], color=month)) +
      geom_point() +
      labs(x = colnames(DailyAQ_merged)[xx],
           y = colnames(DailyAQ_merged)[yy],
           title = paste(colnames(DailyAQ_merged)[yy],"vs",colnames(DailyAQ_merged)[xx]))
    print(p)
  }
}
```
...
#CrossTable(DailyAQ_testLabels, DailyAQ_pred, prop.chisq = FALSE)
...
print(paste("Total Accuracy = ", DailyAQ_pred_accuracy))
...
```{r more kNN analysis}
DailyAQ_cm_result <- data.frame(k=numeric(0), Total.Accuracy= numeric(0), row.names = NULL)
...
for (kval in 4:13) {
  ...
  # initialize a row of the metrics 
  cmt = data.frame(k = kval, Total.Accuracy = DailyAQ_pred_accuracy, row.names = NULL ) 
  DailyAQ_cm_result <- rbind(DailyAQ_cm_result, cmt)
}

xkabledply(DailyAQ_cm_result, "Total Accuracy Summary")
```