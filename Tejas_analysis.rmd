...
rainedLogit.confusion <- confusionMatrix(
...
The accuracy of this logistic model is approx. `r format(accuracy, format = "f", digits = 2)` which shows that out of all the days that our model predicted it would rain on, about `r format(100 * accuracy, format= "f", digits = 2)`% of those predictions were correct.  

The precision value signals that it only rained on `r format(100 * precision, format= "f", digits = 2)`% of the predicted rainy days.  
...
plot(rainedLogitROC)
# rainedLogitAUC <- auc(rainedLogitROC)
```
