...
Model ROC/AUC curve
```{r knn ROC/AUC}
#probabilities <- predict(DailyAQ_pred, type="response")
# knn model using caret
DailyAQ_knn2 <- train(DailyAQ_training, DailyAQ_trainLabels, method="knn")
DailyAQ_knn2
# used model to gather predictions on test
preds_knn <- predict(DailyAQ_knn2, newdata = DailyAQ_test, type = "prob")

# multiclass ROC on test labels
knnROC <- multiclass.roc(DailyAQ_testLabels, preds_knn)
knnROC
```