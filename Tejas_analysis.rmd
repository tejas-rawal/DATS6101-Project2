...
ggplot(DailyAQ_00_22_Yearly_Growth) +
  ...
  labs(title="Average AQI by year in NYC", x="Year", y="Year-over-year Diff (%)")
...
```{r linear model}
aqi_fit <- lm(AQI ~ DATE, data = DailyAQ_merged)
summary(aqi_fit)
...
```{r lm plot}
ggplot(DailyAQ_00_22, aes(x = DATE, y = AQI)) + 
  geom_point(alpha = 0.5, color = "#47ABE9") +
  labs(x = "Year", y = "AQI Value", title = "NYC Daily AQI Values From 2000-2022 And Trend Line") +
  geom_smooth(method = 'lm', formula = 'y ~ x', color = "black", fill="black", se = TRUE)
```


##### Adding `month` as a categorical regressor
In our first analysis, we analyzed linear trends of TMAX over time and determined a slight positive correlation observed over the years 1900-2022. Based on that fit, we hypothesized that seasonality trends had an impact on model performance which would be better explained by 
...
NYweath_Month %>%
  dplyr::select(month, "Avg Max Temp.") %>%
  gather(key="Value", value="Temp.", "Avg Max Temp.") %>%
  ggplot(aes(x=month, y=Temp., fill=Value)) +
  geom_col(na.rm=TRUE, alpha=0.5, color="black", position="identity") +
  labs(title="Average Daily Maximum Temperature By Month", x="Month", y="Temperature (°F)") +
...
ggplot(DailyAQ_00_22_Monthly, aes(x = as.numeric(month), y = aqi_avg)) +
...
ggplot(DailyAQ_00_22_Monthly, aes(x = as.numeric(month), y = aqi_diffRate)) +
...
Let's modify our last model adding `month` as a categorical regressor and our variable-of-interest from the last project - `TMAX` to predict daily AQI.
...
The regression coefficients for TMAX significant and positively correlated, with each degree increase resulting in AQI increasing by a factor of 0.68. All months, when compared to January, have a negative impact on AQI, with September having the largest (negative) slope. The p-value of the model's F-statistic is also significant, allowing us to reject the null hypothesis and conclude that there’s a significant relationship between our chosen predictors and the daily AQI value. However, the $R^2$ for our model is only `.149`, which indicates that only 14.7% of the variation in daily AQI can be explained by TMAX and month.  
